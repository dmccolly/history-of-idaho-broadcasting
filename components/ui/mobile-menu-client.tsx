...
export default function MobileMenuClient({ navigationItems = [], ctaText = "Join Events", ctaLink = "/events" }: MobileMenuClientProps) {
  ...
  // Fallback navigation items if none are provided
  const menuItems = navigationItems.length > 0 ? navigationItems : [
          ...
          <ul className="px-5 py-2">
            ...
            <li>
              <Link 
                ...
                onClick={() => setMobileNavOpen(false)}
              >
                {ctaText} <span className="tracking-normal text-blue-600 group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out ml-1">-&gt;</span>
              </Link>
            </li>
          </ul>
        </Transition>
